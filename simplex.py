...
class Simplex(object):
    def __init__(self, num_vars, constraints, objective_function):
        ...
        print('>>>> TRAEMOS LOS DATOS DEL PROBLEMA A RESOLVER CON SIMPLEX <<<<')
    ...
    def construct_matrix_from_constraints(self):
        ...
        print(num_s_vars)
        print(num_r_vars)
        print('Se formatea las s y las r_vars solo se dejan num_vars con: {}'.format(self.num_vars))

        for expression in self.constraints:
            print('Se evaluan una a una las restricciones: {}'.format(expression))
            if '>=' in expression:
                num_s_vars += 1
        ...
        print('Total de variables: {}'.format(total_vars))
